...
# Trains and predicts a DeNet101 standard/skip/wide detector on Pascal VOC or MSCOCO using 4x GPUs 

MODEL_VAR=$1
if [[ $MODEL_VAR == "std" ]]; then
    MODEL_DESC="PI[2] C.B[384,3] BNA PI[2] C.B[192,3] BNA DNC[128,50] DNS[7,24,0.01,0.1] C.B[2048,1] BNA C.B[1536,1] BNA C.B[1024,1] BNA C.B[768,1] BNA DND[0.5,1,1]"
    BS=32
    BSF=1
elif [[ $MODEL_VAR == "skip" ]]; then
    MODEL_DESC="PI[2] C.B[384,3] SKIP[1] BNA PI[2] C.B[192,3] SKIP[0] BNA DNC[128,50] DNS[7,24,0.01,0.1] C.B[2048,1] BNA C.B[1536,1] BNA C.B[1024,1] BNA C.B[768,1] BNA DND[0.5,1,1]"
    BS=32
    BSF=1
elif [[ $MODEL_VAR == "wide" ]]; then
    MODEL_DESC="PI[2] C[1024,3] SKIP[2] BNA PI[2] C[512,3] SKIP[1] BNA PI[2] C[256,3] SKIP[0] BNA SPLIT DNC[128,200] DNS[7,48,0.01,0.1] C.B[2048,1] BNA C.B[1536,1] BNA C.B[1024,1] BNA C.B[768,1] BNA DND[0.5,1,1]"
    BS=16
    BSF=2
else
...
OUTPUT_DIR=./denet101-$DATASET-$MODEL_VAR
IMAGE_LOADER="images_per_subset=1280,scale=512,crop=512,augment_photo,crop_mode=denet,scale_mode=large"
TRAIN_PARAM="--solver nesterov --epochs 90 --batch-size $BS --batch-size-factor $BSF --learn-rate 0.1 --learn-momentum 0.9 --learn-anneal 0.1 --learn-anneal-epochs 30 60 --learn-decay 0.0001"
...
#print some stuff
echo "Training DeNet-101 ($MODEL_VAR) model on $DATASET with $GPUS"
...
if [ ! -f "./initial.mdl.gz" ]; then
    ...
    if [[ $MODEL_VAR == "skip" ]]; then
        $DENET_DIR/bin/model-modify --input $DENET_DIR/models/imagenet/resnet101.mdl.gz --output initial_skipsrc.mdl.gz --modify-bn 1 0.9 1e-5 --convert-bn-relu --use-cudnn-pool  --class-num $CLASS_NUM --image-size 512 512 --layer-remove 3 --layer-insert 11:SKIPSRC.X[0] 18:SKIPSRC.X[1] > ./modify_skipsrc.out 2> modify_skipsrc.err
        ...
        $DENET_DIR/bin/model-modify --input $DENET_DIR/models/imagenet/resnet101.mdl.gz --output initial_skipsrc.mdl.gz --modify-bn 1 0.9 1e-5 --convert-bn-relu --use-cudnn-pool  --class-num $CLASS_NUM --image-size 512 512 --layer-remove 3 --layer-insert 7:SKIPSRC[0] 12:SKIPSRC.X[1] 24:SPLIT 37:SKIPSRC.X[2] > ./modify_skipsrc.out 2> modify_skipsrc.err
        $DENET_DIR/bin/model-modify --input initial_skipsrc.mdl.gz --output initial.mdl.gz --layer-append $MODEL_DESC > ./modify.out 2> modify.err

    else
        $DENET_DIR/bin/model-modify --input $DENET_DIR/models/imagenet/resnet101.mdl.gz --output initial.mdl.gz --modify-bn 1 0.9 1e-5 --convert-bn-relu --use-cudnn-pool --class-num $CLASS_NUM --image-size 512 512 --layer-remove 3 --layer-append $MODEL_DESC > ./modify.out 2> modify.err
    fi
fi
